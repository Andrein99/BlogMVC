...
namespace BlogMVC.Servicios
{
    public class AnalisisSentimientosOpenAI : IAnalisisSentimientos
    {
            ...
            foreach (var comentario in comentariosPendientesDeAnalisis)
            {
                var request = new
                {
                    ...
                    body = new
                    {
                        ...
                        messages = new[]
                        {
                            new { role = "system", content = "Eres un analista de sentimientos. Devuelve una puntuación de 1 a 5 según la emoción expresada en el mensaje. Sólo debes retornar la puntuación y nada más." },
        ...
        private async Task<string> EnviarBatch(string inputFileId)
        {
            ...
            var response =
                await httpClient.PostAsync("https://api.openai.com/v1/batches", content);
            var jsonResponse = await response.Content.ReadAsStringAsync();

            var batchId = JsonDocument.Parse(jsonResponse)
                .RootElement
                .GetProperty("id")
                .GetString();
            return batchId!;
        ...
        public async Task ProcesarLotesPendientes()
        {
            var lotesSinProcesar = await context.Lotes
                .Where(x => x.Status != "failed" && x.Status != "completed" && x.Status != "expired").ToListAsync();
            ...
            foreach (var lote in lotesSinProcesar)
            {
                ...
                if (status == "completed")
                {
                    ...
                    var respuestaContenidoArchivo = await
                        httpClient
                        .GetAsync($"https://api.openai.com/v1/files/{outputFileId}/content");
